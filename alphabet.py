...
class Symbol():
    ...
    def print(self, use_ascii = False):

        if use_ascii and self.id != -1:
            print(chr(self.id))
        else:
            print(self.id)

    def get_id(self, use_ascii = False):
...
class Alphabet():
    ...
    def __init__(self, size = NUM_PRINTABLE_CHARACTERS, create_ascii = False):
        self.size = size
        self.dictionary = np.empty(shape = self.size, dtype = Symbol)

        if self.size <= NUM_PRINTABLE_CHARACTERS and create_ascii:
            self.use_ascii = True
            self.create_ascii_alphabet()
        else:
            self.use_ascii = False
            self.create_alphabet()
            
    def create_ascii_alphabet(self):
        for i in range(0, self.size):
            self.dictionary[i] = Symbol(ord(PRINTABLE_CHARACTERS[i]))
    ...
    def get_symbol_by_ascii(self, ascii_id):

        for i in range(0, self.size):
            if self.dictionary[i].id == ascii_id:
                return self.dictionary[i]
    ...
    def print(self):
        print("-------------------- ALPHABET --------------------")
        print("NUM SYMBOLS IN ALPHABET: %d" % self.size)
        for symbol in self.dictionary:
            symbol.print(use_ascii = self.use_ascii)
